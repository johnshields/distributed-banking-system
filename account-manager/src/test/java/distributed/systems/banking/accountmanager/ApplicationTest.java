...
class ApplicationTest {
    ...
    void buildConsumerPropertiesSetsExpectedConfig() {
        Properties properties = Application.buildConsumerProperties("localhost:9092", "valid-transactions-group");

        assertEquals("localhost:9092", properties.get(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG));
        assertEquals("valid-transactions-group", properties.get(ConsumerConfig.GROUP_ID_CONFIG));
